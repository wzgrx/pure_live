...
- **录制工具目标守卫已修订**：[工具审计](ANDROID_RECORDING_GUARD_AUDIT_2026_09_07.md)。离线复现非目标前台点击和daemon失败重放，现固定Serial、核验25102RKBEC/myron、前台丢失立即停止；平台选择按实际标签逐次观察，支持Picarto和隐藏/排序。67个守卫/导航场景、7个包装器离线测试及相邻静态检查通过，真实ADB命令0次。代理配置/清理脚本仍待修，暂不运行整套国外录制；复用现有候选，不重构建。

- **新 Android 候选 2006c044 已构建、尚未安装**：[构建与测试审计](PICARTO_ANDROID_CANDIDATE_AUDIT_2026_09_07.md)。包含 Picarto、录制卡片布局和此前累计修复。完整测试首次 1581 通过/1 项期望列表遗漏失败，补齐 Picarto 后相关 2/2 通过，全库分析无诊断；补跑真实接口 42/42 通过。复用未变应用输入证据构建 Debug，APK 287,023,145 B、Manifest 6121、16 个原生库及 16 KB 对齐通过。手机前台仍为其他测试应用，未覆盖安装或输入；旧 af88a032 回滚包已保留。原生录制脚本的固定序号、自动改目标与抢回前台问题已按上项修订，代理生命周期仍待闭合。不是一次完整全绿门禁，也不是原生 PASS 或发布。
