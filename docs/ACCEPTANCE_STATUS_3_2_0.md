...
- **新 Android 候选 2006c044 已构建、尚未安装**：[构建与测试审计](PICARTO_ANDROID_CANDIDATE_AUDIT_2026_09_07.md)。包含 Picarto、录制卡片布局和此前累计修复。完整测试首次 1581 通过/1 项期望列表遗漏失败，补齐 Picarto 后相关 2/2 通过，全库分析无诊断；补跑真实接口 42/42 通过。复用未变应用输入证据构建 Debug，APK 287,023,145 B、Manifest 6121、16 个原生库及 16 KB 对齐通过。手机前台仍为其他测试应用，未覆盖安装或输入；旧 af88a032 回滚包已保留。原生录制脚本存在固定平台序号、自动改目标与抢回前台问题，修复工具后再执行。不是一次完整全绿门禁，也不是原生 PASS 或发布。
