...
- **当前 Android 验收输入**：精确构建提交 `944338e4`（产品修订 `8f43f21b`）的 288823157 B arm64 Debug（SHA-256 `8803DFB8…7002BF`）已完成同签名覆盖安装与设备 APK 哈希核对；在 K90 上通过冷/热/重复口令、口令与附件混合输入释放、单 M3U，以及真实 `ACTION_SEND_MULTIPLE` 的 M3U+XMLTV 导入。混合输入没有误导入，多附件的 Provider/EPG 来源保留原文件名且频道/节目各精确一条，插件暂存树清空；IPTV/Hive 已精确恢复，应用停止。候选仍是开发包，不作为稳定版下载入口；本批详见[Android 外部分享接收审计](docs/ANDROID_INCOMING_SHARE_INTAKE_AUDIT_2026_09_13.md)，更广的播放/弹幕/音频/PiP 基线仍见较早累计候选的[原生证据](docs/CURRENT_ANDROID_CANDIDATE_2026_09_12.md)。
