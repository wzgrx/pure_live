...
- **当前 Android 验收输入**：精确 `3e41e848` 的 arm64 Debug 已完成同签名覆盖安装、安装前后状态一致性、Bilibili 播放/弹幕/音频模式/PiP 16/16 冒烟，以及标准流 7/7、抖音竖屏流 9/9 呈现往返；仍是开发候选，不作为稳定版下载入口。详见[当前累计 Android 候选审计](docs/CURRENT_ANDROID_CANDIDATE_2026_09_12.md)。
- **Android 小窗弹幕设置补证**：当前增量候选已在 K90 完成固定预览开/关、控制组收起/展开、开关/滑块具名辅助语义、恢复取消/确认与进程重启持久化；测试器使用双语语义路由和目标页断言，结束后规范 Hive 逐字节恢复、应用停止。A2-04 处于 RUN，Windows GUI、颜色/后续滑块、主弹幕模板、真实小窗与长测继续；详见[基础原生审计](docs/ANDROID_PIP_DANMAKU_SETTINGS_NATIVE_AUDIT_2026_09_12.md)与[无障碍/默认恢复专项](docs/ANDROID_PIP_DANMAKU_ACCESSIBILITY_RESET_AUDIT_2026_09_12.md)。
