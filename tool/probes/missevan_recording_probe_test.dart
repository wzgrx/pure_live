...
void main() {
  test(
    'Missevan production resolver, native segment growth, stop, MP4 commit and independent decode',
    () async {
      ...
      final output = await Directory(p.join(outputRoot!, 'Missevan 录制 ${DateTime.now().microsecondsSinceEpoch}'))
      ...
      try {
        await HttpOverrides.runWithHttpOverrides(() async {
          ...
          try {
            ...
            final directory = await site.getRecommendRooms();
            expect(directory, isNotEmpty);
            final author = directory.first.roomId!;
            final transport = Platform.environment['PURELIVE_MISSEVAN_RECORDING_TRANSPORT'] ?? 'hls';
            ...
            expect(source.invalidAt, isNotNull);
            final current = LiveRecordTask.fromRoom(
              LiveRoom(platform: 'missevan', roomId: author, nick: 'Missevan probe'),
            )..outputDir = output.path;
            task = current;
            final headers = await FFmpegHeaderFactory.build(platform: 'missevan', roomId: author);
            ...
            final summary = {
              'probe': 'missevan-production-native-recording',
        ...
        fail('Missevan recording probe failed at $stage (${error.runtimeType})');
      } finally {
    ...
    skip: Platform.environment['PURELIVE_MISSEVAN_RECORDING_PROBE'] != '1',
