...
- **新增平台接入进度（源码未发布）**：AcFun 已接入热门、官网分类、作者搜索、分享链接、播放/录制输入和在线人数开关；真实官网搜索验证了稀疏分页，98 个结果按 20/20/20/20/18 连续返回。远端弹幕尚未接入，应用内明确提示；完整录制输出及实际观看验收继续进行，当前 v3.1.8 安装包不含此功能。详见 [AcFun 接入审查](docs/ACFUN_NAVIGATION_AUDIT_2026_09_05.md)。
