...
| A2-04 | NR | 小窗弹幕固定预览/双栏预览实时更新，保存/恢复默认与模板状态一致 | 源码已用共享指标统一预览与实际 Overlay 的字号、速度、轨道高度、表情尺寸和防重叠间距；修改前 3 PASS / 1 FAIL，修订后五文件 26/26、全库 analyze 无问题。Android 系统 PiP、Windows 小窗、重启持久化与长时性能仍待当前最终候选，详见 `docs/PIP_DANMAKU_PREVIEW_FIDELITY_AUDIT_2026_09_11.md` |
