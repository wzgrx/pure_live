...
| [#845 斗鱼 71415 大量弹幕缺失](https://github.com/liuchuancong/pure_live/issues/845) | 原版 WebSocket 单帧只解一个包会漏掉合并包；维护分支已修复。剩余差异来自隐藏的平台消息过滤和默认启用的全平台相似文本过滤 | 对 71415 做两次独立 60 秒原始捕获：分别收到 436/426 个协议包、69/43 条 `chatmsg`；旧解析器接受 63/38 条，只排除 6/5 条同时缺少 `dms` 和 `if` 的消息。被排除样本具有连续 UID、生成式昵称及共同 CID 前缀，符合疑似自动消息特征；但当前 bililive-go、biliup 都按非空 `chatmsg` 接收，并未把 `dms`/`if` 当作可见性协议 | 多包、房间号与空文本门禁保持不变；疑似自动消息过滤改为默认开启的用户可见开关，关闭后得到完整原始房间聊天。全平台模糊相似过滤改为默认关闭，避免忙碌房间的合法短消息二次丢失；旧备份中的显式选择仍保留 |
...
- #767 Windows 4K/高 DPI GPU 占用仍需要报告环境相同的物理 4K/150% 显示器
  做呈现链路采样。`tool/sample_windows_runtime.ps1 -IncludeGpu` 已补齐进程 3D、
  Video Decode、Video Processing、Copy、总引擎及独显/共享显存采样，并记录显卡
  驱动、当前分辨率与刷新率；本机 3840×2400@200Hz 的短时空闲样本已验证采样链路。
  当前维护分支已有 viewport 限制、刷新率策略和 Windows 空闲资源回落证据，但尚无
  150% 缩放下真实播放、滚动和退出回落的等价采样，因此这些证据仍不足以宣布该
  Issue 完成。
