...
最新范围总览：[当前候选、剩余账目和收口顺序](ACCEPTANCE_STATUS_3_2_0.md)。最新已构建 Android Debug 为 `c442380c`，尚未安装且不含本批 TLS 连接层；手机最近已核验安装为 `80b7431c`。下方 `1aa6886f`、`2006c044` 均为历史候选；[原生审计](PICARTO_ANDROID_NATIVE_AUDIT_2026_09_07.md)记录 Picarto 实际视频画面与 22.082 秒短录文件，但完整解码及自动代理收尾失败，整轮未通过。后续手机代理、reverse、进程及唤醒锁清理已验证。候选[分层门禁](PICARTO_ANDROID_CANDIDATE_AUDIT_2026_09_07.md)仍是首次1581/1、夹具修正后相关2/2及接口42/42，不外推全绿。历史账本62大项中42项RUN/NR不等同当前Bug数或完成百分比。
