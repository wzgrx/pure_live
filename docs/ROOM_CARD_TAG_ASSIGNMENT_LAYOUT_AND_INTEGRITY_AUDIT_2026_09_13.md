...
确定性测试最终 **4/4 PASS**，相邻十文件 **131/131 PASS**，全库 analyze 为
`No issues found`。同提交 arm64 Debug 已在
`25102RKBEC / myron / Android 17` 上保留数据覆盖安装；真实热门 Bilibili 房间完成长按、未关注
引导、创建标签、自动选中、确认和重新打开保持选中的完整路径，进程日志无 FATAL/ANR，规范 Hive
最终逐字节恢复。该证据更新 A1-05/A2-01 的当前覆盖，但两组仍为 `RUN`；宏观计数保持
...
| APK | `288827213` B |
| SHA-256 | `88864CF52952C4505E79E4FA2E304DC8845905A1D77E04276F1B242D7BAB8C65` |
| ABI / 原生库 | `arm64-v8a`；16 个库；最小 ELF LOAD `0x4000` |
| Flutter 资源 | 1262 项 / `206860245` B |
| 构建记录 | `local-artifacts/build-records/20260912T191523805Z-build-androidarm64-debug.json` |

## K90 原生验收

最终结果：
`local-artifacts/diagnostics/android-room-tag-assignment-20260913T033448846/summary.json`。
...
- 当前房间原本未关注，标签入口正确进入关注确认，再到标签选择器。标题宽 984 px；新增、取消、确认
  控件分别为 `144×144 / 192×144 / 210×144`。
- 两个编辑框均为 `825×144`；数字夹具标签 `26091303344885` 创建后语义
  `selected=true`，描述 `31415926` 可见。确认分配后再次长按同一卡片并打开选择器，标签仍为
  `selected=true`。
- 同一应用 PID `30956` 的日志没有 FATAL、ANR、Fatal signal 或 SIGABRT；业务与清理门禁全部为
  `true`。
...
关键截图同目录保存为 `room-dialog-first.png`、`tag-selector-initial.png`、
`tag-add-form-filled.png` 和 `room-dialog-second.png`。
...
身份与前台保护、覆盖安装、语义控件、进程日志、Hive 精确恢复和结束停进程；该测试已加入
`tool/local_ci.ps1`。
