...
- **当前 Android 验收输入**：精确构建提交 `2cc1b56d`（产品修订 `8f43f21b`）的 288823157 B arm64 Debug（SHA-256 `32D6B783…862F7B`）已完成同签名覆盖安装与设备 APK 哈希核对；在 K90 上通过冷启动/运行中分享口令、重复口令抑制、口令与附件混合输入释放，以及 M3U 内容 URI 导入到唯一 SQLite 频道/Provider。混合输入没有误导入，Provider 保留发送方原文件名，插件暂存树清空；IPTV/Hive 已精确恢复，应用停止。候选仍是开发包，不作为稳定版下载入口；本批详见[Android 外部分享接收审计](docs/ANDROID_INCOMING_SHARE_INTAKE_AUDIT_2026_09_13.md)，更广的播放/弹幕/音频/PiP 基线仍见较早累计候选的[原生证据](docs/CURRENT_ANDROID_CANDIDATE_2026_09_12.md)。
