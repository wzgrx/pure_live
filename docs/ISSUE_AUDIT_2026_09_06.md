...
新增[实际画质审计](DOUYU_QUALITY_ACK_AUDIT_2026_09_06.md)：真实适配器 HTTP 夹具红测复现响应 `rate` 丢失和混合档位线路；修复确认信息传递、按档分组、录制游标，五组 50/50 与一次 analyze 通过。独立审查另发现未知/列表外档位在消费层仍回退请求名称，已作为发布前待修项。
匿名实网样本请求原画/高清均返回相应档位；没有复现 #853 的动态画面模糊，故该 Issue 整体现象继续按 `not-reproduced` 跟踪，不把局部合同修复等同于用户投诉已关闭。

以下表格保留初审时的范围和证据；斗鱼条目后续以上述审计为准。
