...
当前累计源码已经落到同签名 Android 候选并覆盖安装，安装前数据范围逐文件一致，基础播放/弹幕/音频模式/PiP 冒烟通过，首页软件音量按键路由补证通过。它仍是 Debug 验收输入，不替代完整全库质量、全部设置页、22 个平台、录制、故障注入、长测和正式发布门禁；宏观状态保持 **20 PASS / 33 RUN / 9 NR，共 42 组未闭环**。
