...
- **09-13 Android MPV 默认音频输出已由 OpenSL ES 单点改为 AudioTrack 优先的有序回退**：[专项审计](ANDROID_AUDIO_OUTPUT_BACKEND_AUDIT_2026_09_13.md)。依赖侧固定提交在 Android 实机默认写入 `ao=opensles`，而当前 arm64 libmpv 同时包含 AudioTrack、AAudio 与 OpenSL ES；`fec7eae9` 改为 `audiotrack,aaudio,opensles,`，过滤 Android 设置页的异平台驱动并保留专家显式选择。相邻与 focused CI 均 **28/28 PASS**、全库 analyze 无问题；288822657 B arm64 Debug 已保留数据覆盖 K90，设备 APK SHA-256 与 `DE7DE185…FFAE0E` 一致。真实 Bilibili 5/5 次视频→纯音频→退出的 14/14 门禁通过，新 PID 日志尾窗中 AudioTrack 相关 152 行，OpenSL ES、unknown-key 和 `setVolume -19` 均为 0；11 条 Binder death-recipient 告警在本机日志中与 `c2.qti.avc.decoder` 的分配/启动相邻，Binder NDK 告警条件及 Codec2 AIDL 客户端调用形态提供高可信平台媒体栈归因线索，因缺设备系统映像的带符号栈而保持边界表述，不以关闭硬件解码换取静默。后续显式 Widget 测试先捕获退役标签残留，`09315462` 补齐 Android 五项弹窗，最终 16/16 与 analyze 通过；原生设置路由再暴露缓存坐标假成功，`3bfda37b` 改用双语实时语义和目标页断言，K90 音频菜单 6/6 通过并恢复设置副本。新增可复用播放器内核语义路径及带精确 Hive 保护的音频输出设置 smoke，五种专家 `--ao` 均完成即时选择与跨进程持久化，弹窗始终只有五项 Android 后端；另检测并校正前一只读路由的应用启动写入，设备 Hive 最终回到 `19F40EA9…D1F050`。这组补证只覆盖设置 UI/持久化，逐后端媒体输出继续。瞬态线程退出竞态由 `da8c15b1` 修订并经完整重跑验证。A7-04 保持 RUN，宏观保持 **20 PASS / 40 RUN / 2 NR**、42 组未闭环；Astra Light 0 次。
...
当前账本已有 20 项 PASS、37 项 RUN、5 项 NR，共 **42 个历史大项尚未闭环**。一行通常含多个动作或平台组合；此外还有单独的平台能力表、长时验证及发布门禁。部分 RUN/NR 已在后续审计产生增量证据，尚需对齐；部分 PASS 属于旧版本/旧手机。由此既不推导“当前还有 42 个 Bug”，也不推导完成百分比或最终候选已有 20 项通过。
