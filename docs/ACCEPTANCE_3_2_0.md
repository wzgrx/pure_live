...
- **09-13 Android 外部分享口令/附件及 Provider 边界已完成当前 K90 原生闭环**：[专项审计](ANDROID_INCOMING_SHARE_INTAKE_AUDIT_2026_09_13.md)。`269caa91`～`8f43f21b` 接通初始/运行中分享并逐项释放插件附件；`98f5472a` 再隔离单 URI 异常，以 UTF-8 字节限制并逐 code point 截断显示名。此前联合回归 110/110、最终路径 15/15 和全库 analyze 已通过；本增量直接 11/11、Built-in Kotlin 审计、精确 Kotlin/Manifest 构建与原生门禁通过。288823157 B / `55F94C97…F2B3` arm64 Debug 在 K90 完成冷/热/重复口令、混合附件、单 M3U、真实 M3U+XMLTV 多附件，以及类型/查询异常与超长中文/emoji/控制字符名。异常后的两份 M3U 各精确入库一次，查询异常按 URI basename 回退，长名安全化为 175 UTF-8 字节 basename；暂存树为空、无 FATAL/ANR。同源码 127835744 B / `4BF85571…6B82` R8 Release 测试包又通过冷/热/重复口令、混合附件、单 M3U 和 Debug 探针排除；该包使用 Debug 证书并明确标记 `debug-signed`，最终正式签名候选仍待生成。两轮 IPTV 树逐文件恢复，Hive 回到 `91D6BAC5…6128F`，应用停止、桌面/stay-awake 复原。A1-05/A2-01 保持 RUN，Windows、真实第三方发送方和最终正式签名候选继续；宏观保持 **20 PASS / 40 RUN / 2 NR**、42 组未闭环，Astra Light 0 次。
