...
- Current single-project testing override (2026-09-05): the user has suspended three-task rotation. Use the same wrapper with `-NoRotation`, preserving wake, foreground checks and cleanup. Prefer Android when its network transport is online, otherwise test Windows. Do not merge upstream code during this audit; read upstream Issues for comparison only. Resume the shared lease only if the user resumes shared-device testing.
