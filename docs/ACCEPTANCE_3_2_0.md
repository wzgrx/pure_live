...
最新范围总览：[当前候选、剩余账目和收口顺序](ACCEPTANCE_STATUS_3_2_0.md)。最新 Android 候选 `2006c044` 已保留数据覆盖安装；[原生审计](PICARTO_ANDROID_NATIVE_AUDIT_2026_09_07.md)记录 Picarto 实际视频画面与 22.082 秒短录文件，但完整解码及自动代理收尾失败，整轮未通过。后续手机代理、reverse、进程及唤醒锁清理已验证。候选[分层门禁](PICARTO_ANDROID_CANDIDATE_AUDIT_2026_09_07.md)仍是首次1581/1、夹具修正后相关2/2及接口42/42，不外推全绿。历史账本62大项中42项RUN/NR不等同当前Bug数或完成百分比。
