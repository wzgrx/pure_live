...
最新范围总览：[当前候选、剩余账目和收口顺序](ACCEPTANCE_STATUS_3_2_0.md)。当前干净 `af88a032` 已完整通过 1522 测试/42 接口并构建 Android 候选，保留数据完成覆盖安装；首页横竖屏及左侧导航滚动已有新包证据，其余原生工作继续。历史账本 62 大项中 42 项 RUN/NR 不等同当前 Bug 数或完成百分比。

## 辅助页面原生补证与文案（2026-09-07）

[Issue当前状态](ISSUE_AUDIT_2026_09_07.md)与[平台扩展差距](PLATFORM_EXPANSION_AUDIT_2026_09_07.md)已按实时API和源码注册表刷新：历史42个未闭环大项之外，另有17个未注册候选分组；Picarto公开目录、详情和HLS通过Clash返回200，仍待生产适配器和完整验收，不记作已支持。

[录制中心窄屏布局](RECORDER_PAGE_LAYOUT_AUDIT_2026_09_07.md)：真实页面4个红测复现后，封面/详情、平台信息、统计与动作分组适配实际宽度及文字缩放；28项页面动作和相邻录制合同共90/90通过，analyze无诊断。没有改写FFmpeg或处理中启动的等待保护；此批尚未构建/上机，不借用旧候选原生通过结论。
