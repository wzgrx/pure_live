...
- **新增Picarto源码适配**：[接入与验证](PICARTO_ADAPTER_AUDIT_2026_09_07.md)。公开目录、详情、HLS画质、严格录制输入、重新取流和第4代配置迁移已实现；11文件定向回归115/115，风格修正后30个相关用例加1个生产代理探针31/31，范围分析无诊断。真实HTTP取得目录10条、声明720p30的画质和285B媒体列表；不是原生播放/录制文件证据。当前源码11个直播站点加IPTV；原17个扩展候选中Picarto进入实现、另16组仍未注册，Picarto完整分类/原生搜索/弹幕与原生验收继续。没有构建、设备输入或发布。
...
- **前批源码：录制卡片窄屏布局修复**：[页面审计](RECORDER_PAGE_LAYOUT_AUDIT_2026_09_07.md)。真实页面在320宽度、中英文1/2倍文字下稳定复现4个溢出场景；封面/详情和底部动作现按约束换行。28项页面操作加相邻录制生命周期共 **90/90通过，analyze无诊断**。保留处理中启动等待旧封装的既有意图合同，不重写状态机。尚未构建或上机，下面Android/Windows候选不含此新布局；完整门禁与原生验收继续。
