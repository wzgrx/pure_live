...
[Windows CPU对照诊断](WINDOWS_CPU_CONTEXT_AUDIT_2026_09_07.md)：Pure Live空页面约0.958%/0.045%/0.924%；后续相同SDK无插件静态模板Debug约0.243%/0.013%/0.288%、Release约0.306%/0.0067%/0.308%（可见/最小化/恢复）。修正仅业务或仅Debug开销的解释，保留产品自身Release及原生路径定位待办；不是应用修复或性能PASS。
