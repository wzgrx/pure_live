...
- **当前 Android 验收输入**：精确产品提交 `cc3ab5b2` 的 arm64 Debug 已完成同签名覆盖安装与设备 APK 哈希核对；在 K90 上重新通过小窗弹幕开关/默认恢复，并在真实 Bilibili 直播间通过主弹幕模板保存→修改开关和计数器→恢复→进程重启往返。候选仍是开发包，不作为稳定版下载入口；更广的播放/弹幕/音频/PiP 基线仍见较早累计候选的[原生证据](docs/CURRENT_ANDROID_CANDIDATE_2026_09_12.md)。
