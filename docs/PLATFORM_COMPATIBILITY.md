...
2026-09-11 按 `lib/core/sites.dart` 核对：当前源码注册 **21 个直播站点 + IPTV，共22个适配器**。这是源码注册数量，不是已发布包或完整验收数量。小红书、niconico 和微博已接入应用入口；微博应用合同、大字号目录与搜索动作的定向验证已闭环，原生整体验收继续，见[微博接入记录](WEIBO_APPLICATION_INTEGRATION_2026_09_11.md)。当前候选与完整剩余范围以[验收状态](ACCEPTANCE_STATUS_3_2_0.md)为准。
...
“全部”搜索并发请求各原生平台，但按单个平台完成顺序渐进显示，某个平台超过 12 秒会被标记为本轮部分失败，不再阻塞其他结果。搜索页生命周期内复用同一组适配器，Twitch 等游标分页状态不会因每次读取平台列表而丢失。网页继续搜索可从 Bilibili、斗鱼、虎牙、抖音、快手、网易 CC、Twitch、SOOP、YY、AcFun、Picarto 和 TwitCasting 的已支持直播间链接识别“平台 + 房间号”；搜索/分类页和相似伪装域名会被忽略。
