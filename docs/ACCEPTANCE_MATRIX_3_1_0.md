...
| A2-04 | RUN | 小窗弹幕固定预览/双栏预览实时更新，保存/恢复默认与模板状态一致 | 源码已用共享指标统一预览与实际 Overlay 的字号、速度、轨道高度、表情尺寸和防重叠间距；修改前 3 PASS / 1 FAIL，修订后五文件 26/26、全库 analyze 无问题。09-12 K90 / 当前 `3e41e848` 候选进一步完成 Android 移动固定预览的开→关→重启→开→重启：Switch 数 4→1→1→4→4，禁用遮罩同步变化，两种状态均跨进程保持；测试后规范 Hive `443377` B 以同一 SHA-256 精确恢复，应用停止。Windows 双栏、逐项样式、默认/模板、实际系统 PiP/小窗和长时性能继续，详见 `docs/PIP_DANMAKU_PREVIEW_FIDELITY_AUDIT_2026_09_11.md`、`docs/ANDROID_PIP_DANMAKU_SETTINGS_NATIVE_AUDIT_2026_09_12.md` |
