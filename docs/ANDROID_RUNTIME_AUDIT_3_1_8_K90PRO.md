...
- cycle 196 用 `tool/android_color_picker_smoke.ps1` 验证统一颜色入口。主题色弹窗明确使用“选择色阶”和 6 位 RGB，不再把色阶标题误写成透明度；加载颜色弹窗显示真实 alpha 滑块、透明棋盘和始终可编辑的 8 位 ARGB。输入 `0x800080DD` 后预览/代码同步，取消并重开恢复原始 `0xFFA0CAFD`，全部命名断言通过。包装器在取得 C 轮后唤醒无密码锁屏设备、测试期间临时保持供电常亮，并在交棒前恢复 10 分钟锁屏策略。证据为 `local-artifacts/diagnostics/android-color-picker-20260905T023013977/summary.json`。
