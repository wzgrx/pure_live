...
| [#851 小窗关闭按钮几秒后消失](https://github.com/liuchuancong/pure_live/issues/851) | 报告为 Android 3.1.2，描述小窗控件自动隐藏后没有关闭入口；目前仅完成报告读取，尚未把截图场景映射到本地控件生命周期 | `not-reproduced`；核对应用内小窗与系统 PiP 的入口、自动隐藏与点击重新唤出，加入控件隐藏/恢复/关闭对照。不要把 Windows PipWidget 当 Android 复现 |
