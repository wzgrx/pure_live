...
void main() {
  ...
  test('version seven adds Kilakila and later Huajiao and disabling survives disk reopen', () async {
    ...
    expect(settings.hotAreasList, ['huya', 'inke', 'kilakila', 'huajiao', 'openrec', 'ttinglive', 'xiaohongshu']);
    expect(settings.siteCatalogMigration.value, 12);
    settings.hotAreasList.remove('kilakila');
    settings.onInit();
    expect(settings.hotAreasList, ['huya', 'inke', 'huajiao', 'openrec', 'ttinglive', 'xiaohongshu']);
    ...
    expect(reopened.hotAreasList, ['huya', 'inke', 'huajiao', 'openrec', 'ttinglive', 'xiaohongshu']);
  });
