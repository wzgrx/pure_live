# 参考平台扩展差距与 Picarto 输入样本（2026-09-07）

- **09-11 当前差距：微博已应用注册**：[接入与 UI 回归](WEIBO_APPLICATION_INTEGRATION_2026_09_11.md)。有限目录、精确场次查询、分享/外部打开、配置迁移及播放/录制源消费者已接通；253 项定向闭环、24 文件严格分析通过，并修订热门页大字号卡片溢出。当前 **21 个直播站点 + IPTV，6 组未注册**：YouTube、Bigo、战旗、一直播、企鹅电竞、浪 Live。此前微博留存媒体的时钟缺陷已按[音频 PES 证据](RECORDING_AUDIO_PES_CLOCK_2026_09_11.md)完成限定原生修订；下方旧状态为取证历史。完整平台能力、双端验收和 3.2.0 发布继续，未缩减目标。

- **09-10 录制拼接时钟缺陷已定位，尚未修复**：[同输入逐帧对照与离线候选](RECORDING_CONCAT_CLOCK_DRIFT_2026_09_10.md)。实际 FLV/TS/MP4 的 255 视频帧和 278 音频帧内容顺序一致，但生产拼接额外引入约 90 毫秒阶跃；离线候选已消除该样本阶跃，仍待项目原生内核、确定性多场景及生产接线验证。

- **09-10 微博适配器与录制证据续查**：[适配器源码检查点](WEIBO_ADAPTER_CHECKPOINT_2026_09_10.md)、[同输入诊断](WEIBO_RECORDING_INPUT_EVIDENCE_2026_09_10.md)。旧 MP4 完整解码通过，但两处视频时间间隙未归因；新增显式有界 FLV/TS 留存，87/87 定向通过。平台注册、双端应用验收及 3.2.0 发布仍待完成。

- **09-10 微博直播元数据层已验证**：[微博/一直播审计](WEIBO_METADATA_API_AUDIT_2026_09_10.md)。实际目录和在播详情接通；HLS 字段实为 FLV，保留访问门槛/回放/播放开关与身份校验，不猜画质。154/154 定向、67/67 含真实探针（去重 155，新增确定性 54）、四文件严格分析通过。公开媒体仅 FLV 前缀，完整播放/录制与应用注册仍待续；一直播 DNS/代理访问失败不判停运。20 直播站点 + IPTV、7 组未注册、42 组未闭环保持，未操作手机/构建/发布。

- **09-10 Windows 音量控件替换已修订**：[控件审计](VOLUME_WIDGET_LIFECYCLE_AUDIT_2026_09_10.md)。补齐重绘后的 6 项 Widget 基线全部失败；修订控制器订阅/迟到初值/悬停归属，避免移动端默认音量误写 Windows 当前房间。四文件 89/89、两文件严格分析通过（新增 12 项），非实际声卡或原生候选验收；48154d15 APK 不含近期音量修订，完整目标与待验收计数保持。

- **09-10 音量生命周期修订**：[音量审计](VOLUME_LIFECYCLE_AUDIT_2026_09_10.md)。实际控制器复现 4 项缺陷：退出后写入、初始快照覆盖偏好、显示 Rx 未更新、迟到 setter 发布；修订归属/代次及平台等待上限后，五文件 206/206、五文件严格分析通过（新增 16 项）。保留夹具两项错误断言记录；非 #858 物理按钮根因结论，未操作手机。48154d15 已归档 APK 不含该修订，后续候选须按新 SHA 验证。

- **09-10 累计 Android 候选已构建并核验**：[完整候选证据](ANDROID_CUMULATIVE_CANDIDATE_2026_09_10.md)。干净 48154d15：全库 analyze 无问题、4072/4072 Flutter、42/42 指定公开接口；arm64 Debug APK / 3.1.8+4121 / Manifest 6121 / 288640934 B，哈希 B33B83C8…DE782416。包内容、16 KB 对齐、签名与旧候选保留核验通过；78 项原生场景仍全部 not-run，未安装/操作手机/发布，继续代码审查及全范围验收。

- **09-10 全量失败已定位到旧夹具并完成定向修订**：[累计门禁续记](CUMULATIVE_QUALITY_AND_SOURCE_SYNC_2026_09_10.md#第二轮完整门禁及分页引擎夹具修订)。a2b539cb 全量为 4069 PASS / 3 FAIL；独立复现相同失败。CC 视口准备与默认引擎回退前置状态修订后，六文件 273/273、两文件严格分析通过；保留首轮 Widget/Dio 虚拟时钟等待的失败记录。业务不变，待新干净提交完整回归；尚无新 APK / 原生 PASS，源代码每批同步 GitHub。

- **09-10 Issue 增量审查**：[Issue / 音量链路记录](ISSUE_AUDIT_2026_09_10.md)。维护仓库 open 0、上游 18；新增 #858 Android 音量键、#859 iOS 抖音全屏闪退均待复现，#849 新评论无新增诊断附件。累计门禁在 a2b539cb 上实际为 analyze 无问题、Flutter 4069 通过 / 3 失败，公开接口与 APK 构建尚未到达；失败夹具修订及定向验证进行中。未改变原生验收与全平台 3.2.0 范围。

- **09-10 累计门禁与 GitHub 同步**：[门禁/同步审计](CUMULATIVE_QUALITY_AND_SOURCE_SYNC_2026_09_10.md)。337 个积压提交及持续同步规则已推送，源码同步与 3.2.0 发布分开。全库分析捕获一个测试夹具遗漏 hlsPrefetch 的签名错误，补齐后 11/11 与严格分析通过；首轮完整门禁仍记失败，完整套件/公开接口及累计 APK 待续。新计划 78 个 Android 场景全部 not-run；本轮仅核对手机身份，未安装/切前台。20 直播站点 + IPTV、7 组未注册和 42 组未闭环保持。

- **09-10 搜索取消与退出栅栏已修复**：[搜索审计](SEARCH_CANCELLATION_LIFECYCLE_AUDIT_2026_09_10.md)。修复空草稿换平台残留旧结果、Windows 检测退出后迟到弹窗；niconico 搜索接通独立传输 token，旧适配器停止等待并消费迟到错误。修改前两项 Widget 稳定失败；最终七文件 142/142（新增 15 项）、五文件严格分析通过，保留首次测试帧调度错误记录。仍有旧平台底层取消/游标归属、累计候选及双端原生验收待续；20 直播站点 + IPTV、7 组未注册、42 组未闭环保持，未操作手机/构建/发布。

- **09-10 多画面与录制发现归属已接通**：[归属审计](MULTIVIEW_RECORDER_DISCOVERY_OWNERS_AUDIT_2026_09_10.md)。每格、录制任务及预取独立取消并等待临时发现清理；缩容先取消所有移除格。另复现并修复格子重建复用 epoch 导致旧结果创建播放器的竞态。首轮 178/178，最终多画面 81/81、其余 98 项同业务输入复用，去重 179 项（新增 13）；九文件严格分析通过，保留分析与复现失败记录。搜索/其他传输取消、真实多路/长录及双端 GUI 待续；20 直播站点 + IPTV、7 组未注册、42 组未闭环保持，未操作手机/构建/发布。

- **09-10 主播放与工具箱画质发现已接通主动取消**：[请求归属审计](QUALITY_DISCOVERY_CANCELLATION_AUDIT_2026_09_10.md)。修改前两项确定性失败；新增可选取消合同，niconico 发现会话在退出/换请求/超时后取消并等待清理，保护新播放器不受旧销毁影响。最终八文件 148/148（新增 12 项）、六文件严格分析通过；保留编译失败记录。多画面每格、录制任务/预取及搜索取消仍待续，非真实网络/双端原生验收；20 直播站点 + IPTV、7 组未注册、42 组未闭环保持，未操作手机/构建/发布。

- **09-10 niconico 应用注册/分享/搜索/目录迁移已接通**：[接入审计](NICONICO_APPLICATION_INTEGRATION_AUDIT_2026_09_10.md)。迁移 13 保留顺序和隐藏选择，严格 watch 链接接入共用入口；新增 27 项，去重 306 项通过覆盖、22 文件严格分析通过，含双语窄/宽屏 200% 字号真实目录 Widget。两项旧迁移期望与一次测试时钟停滞均保留记录并复测；页面画质发现主动取消、真实多画面/长录和双端原生验收仍待续。当前 **20 直播站点 + IPTV，7 组未注册，42 组验收未闭环**；未操作手机/构建/发布。

- **09-10 niconico 原生分类目录与关键词搜索已适配**：[目录审计](NICONICO_DIRECTORY_AUDIT_2026_09_10.md)。依据实际官方脚本及响应区分每页 70 的 recent 和每页 40 的 search，接入七分类、公开卡片、取消和服务端分页；空搜索不冒充全站目录。127/127（含新增 35 项）、六文件严格分析及外部探针 2/2 通过，回放 13 响应/415 条，十次实际目录/搜索请求成功。尚待平台注册/搜索与分享入口/取消链/迁移及双端 GUI、长录；仍 42 组未闭环、8 组未注册，未操作手机/构建/发布。

- **09-10 niconico 元数据与实际画质适配已验证**：[站点适配审计](NICONICO_SITE_ADAPTER_AUDIT_2026_09_10.md)。临时发现会话返回前关闭，实际分辨率/码率生成公开配方；补齐动态封面、头像及访问提示。最终新增确定性 39 项与生产探针共 40/40，七文件严格分析通过，125 项相邻通过证据复用。真实节目返回三档画质，seat 清理且 Cookie 为 0；未注册目录/导航，未媒体解码或双端 GUI/长录。保持 42 组未闭环、8 组未注册；未操作手机/构建/发布。

- **09-10 多画面已接入每格独立的会话型输入**：[多画面审计](MULTIVIEW_OWNED_INPUT_AUDIT_2026_09_10.md)。公开配方穿过解析、状态、画质切换及实际 cell owner，每次原生打开独立创建；换源取消、迟到清理、过期会话主动恢复与移除后的状态隔离接通。修复本轮稳定复现的恢复覆盖新源竞态；最终七文件 111/111（含 19 项新增）、五文件严格分析通过。底层解码/seat 使用替身，真实多路解码、自动恢复、平台注册及双端长录/GUI 待续；未操作手机/构建/发布，42 组未闭环、8 组未注册保持。

- **09-10 会话型输入的直链/投屏能力处理已补齐**：[入口审计](OWNED_INPUT_EXPORT_AUDIT_2026_09_10.md)。共用 flow 改用统一解析结果，会话源给出应用内播放/录制提示，不导出旧回退 URL 或私有中继；普通 URL 行为保留。修改前两项确定性失败，最终五文件 78/78（含 14 项新增）及五 Dart 文件严格分析通过。多画面每格会话管理、niconico 导航/注册、真实长录和双端 GUI 待续；未操作手机/构建/发布，19 站点 + IPTV、8 组未注册、42 组未闭环保持。

- **09-10 niconico 公开配方已可绑定录制 owner**：[录制接入审计](RECORDER_OWNED_INPUT_AUDIT_2026_09_10.md)。每次录制重取 watch、独立 seat/relay，保留录制代理与明确选流；录制解析/页面/FFmpeg 管理器和服务接通类型化入口、停止取消与收尾。新增 19 项；最终 17 项与复用的 150 项相邻证据分列，11 文件严格分析通过；确定性源经实际 Windows FFmpegKit 的自有/普通两种控制器路径取得四份完整双轨文件。真实 niconico 录制、导航/导出与平台注册仍待续；19 站点 + IPTV、8 组未注册、42 组未闭环保持。

- **09-10 配方进入站点解析与页面线路状态**：[接线审计](OWNED_INPUT_ROUTE_AUDIT_2026_09_10.md)。公开 recipe 与播放器绑定分层，空导出 URL 不再丢失自有输入，画质切换、恢复和页面悬浮保留类型；实际管理器三类回执及真实线路 Widget 验证通过。新增 18 项、复用前轮 160 项中的相邻证据，最终 5/5 与 16 文件分析通过。录制器 owner、目录/搜索/分享及平台注册待续；19 站点 + IPTV、8 组未注册、42 组未闭环保持。

- **09-10 niconico 配方进入实际播放器管理器**：[类型化状态审计](PLAYER_OWNED_SOURCE_STATE_AUDIT_2026_09_10.md)。以源类型代替非空 URL 判定，恢复/切引擎重新创建独立输入，管理器悬浮快照保留配方。15 项新增回归、170 项相邻复用及六文件严格分析通过；实际管理器向桌面 FFmpeg 提供私有输入，六秒双轨/提交/清理通过，非 GUI 或默认开播时限验收。首次输入失败根因仍待证据；19 站点 + IPTV、8 组未注册和历史 42 组未闭环保持。

- **09-10 niconico 可重建配方接通播放器底层输入事务**：[审计](PLAYBACK_OWNED_INPUT_AUDIT_2026_09_10.md)。每次新 watch/独立 seat，取消等待迟到清理，原生打开前后检查输入有效性；新增 24 项及相邻回归通过。生产调用经传输层向桌面 FFmpeg 提供私有 URL，六秒双轨/解码/清理通过且预取 feed=0。仍待应用状态、录制器、目录/导航与平台注册；19 站点 + IPTV、8 组未注册、历史 42 组未闭环保持。

- **09-10 niconico 生产输入 owner 与官方节目合同推进**：[所有权审计](NICONICO_INPUT_OWNERSHIP_AUDIT_2026_09_10.md)。移除探针临时清理拼接，支持取消/迟到资源、按实例代理；官方节目实际使用无 unama 前缀的 WebSocket 路径，两档 512×288 按码率区分并保留配套音频。183 项定向、另 12 项连接/TLS 通过；生产 owner 在实际 800×450 档位六秒双轨/清理通过。三份前置外部失败保留，不等同平台已注册；当前仍 19 站点 + IPTV、8 组未注册、历史 42 组未闭环，继续应用消费者和双端验收。

- **09-10 niconico 显式画质与配套音轨的保留路径已验证**：[选流审计](HLS_MASTER_SELECTION_AUDIT_2026_09_10.md)。新 master 选择合同不猜画质/语言、不丢外置音频；固定 3888→3893 源窗口对照与最终 92 项定向通过，独立真实短录 1/1 达到完整六秒双轨。该次使用桌面 FFmpeg 与生产中继，尚非应用消费者或 Android 验收；未选流 ABR 失败保留。当前仍 19 站点 + IPTV、8 组未注册、历史 42 组未闭环，继续会话/选流的应用接线与跨窗口验证。

- **09-10 niconico 运行时凭据与真实媒体已接通，短录双轨尚未闭环**：[中继/缺片审计](NICONICO_RELAY_AUDIT_2026_09_10.md)。71 项生产回归、5 项合同回归、六文件分批严格分析通过；列表/密钥/分片已经生产中继得到可解码 720p H.264 + AAC。两次成品分别双轨过短和音轨过短，已保存包级负例并定位短窗口内音频序号跳过，未以一次容器时长通过掩盖缺失。仍 19 直播站点 + IPTV、8 组未注册、历史 42 组未闭环；消费者所有权、选流/保留、导航/平台接入与双端验收继续，手机与候选保持。

- **09-10 niconico 会话/媒体 Cookie 接入推进**：[会话审计](NICONICO_SESSION_AUDIT_2026_09_10.md)。24 项会话、同输入 84 项、3 项录制 Cookie 兼容分批通过；生产探针 3/3、八文件分批严格分析通过。真实 seat 保活后 HLS 与普通媒体密钥请求成功，关闭后 Cookie 为 0；仍待应用消费者、解码/实录、导航/平台注册及原生验收，19 直播站点 + IPTV、8 组未注册与宏观 42 组保持。

- **09-10 niconico 原生 Dart 元数据与会话入口已验证**：[审计](NICONICO_METADATA_AUDIT_2026_09_10.md)。53/53（含真实 HTTP 1/1）、五文件严格分析通过；匿名 WebSocket 返回 seat/stream 和按路径 Cookie，HLS 主列表 200、三档。不是外部 Streamlink 的移动端包装；播放/录制会话所有权、Cookie 接线及应用注册待续。当前仍 19 直播站点 + IPTV、8 组未注册、42 组验收未闭环；未构建、操作手机或合并上游。

- **09-10 小红书累计 Android 候选与当前备份已核验**：[候选审计](XIAOHONGSHU_ANDROID_CANDIDATE_2026_09_10.md)。干净 d4adcb77 构建 arm64 Debug，288486345 B，签名/16 KB/内容/双语与独立归档通过；未安装。当前旧 APK 哈希匹配已验签备份，新取得两份一致状态快照，227 文件/9 数据库核验通过，迁移输入未变。候选 51 场景全 not-run，手机只读且等待前台窗口；19 直播站点 + IPTV、8 组未注册、宏观 42 组未闭环保持。

- **09-10 小红书真实短链与分享别名已接入**：[分享审计](XIAOHONGSHU_SHARE_LINK_AUDIT_2026_09_10.md)。127c5781 接通短链、dynpath/hina/裸域名与搜索，修复终端点段经 URI/标点处理被误接受；有界跳转不读取主页、笔记、无关或本地目标。214 项通过、末次 82 项复验、六文件严格分析通过，另有真实分享/搜索 1/1。未构建或操作手机；19 直播站点 + IPTV、8 组未注册、宏观 42 组未闭环保持。

- **09-10 小红书真实控制器原生录制两轮通过**：[原生审计](XIAOHONGSHU_CONTROLLER_NATIVE_AUDIT_2026_09_10.md)。4f84013d 探针使用实际注册适配器与录制控制器，两轮自动合并 MP4、完整解码、指标回写和资源释放通过；原生 1/1，最终单文件严格分析通过。预取选项开启但两轮均未准入，按原 relay 路径验收，不代表源完整性或 GUI/Android/长时通过。当前 19 直播站点 + IPTV、8 组未注册、宏观 42 组未闭环；候选、版本和手机保持。

- **09-10 小红书已接入应用，原生验收待续**：[应用审计](XIAOHONGSHU_APPLICATION_INTEGRATION_AUDIT_2026_09_10.md)。69f6db99接通精确房间查询、分享/收藏、质量线路与播放/录制解析，目录迁移12保持用户隐藏选择；320项定向通过，观看值空态末次31项复验及24文件严格分析通过。注册适配器生产探针1/1，HLS 200/426B/3分片；未原生播放或实录。当前19直播站点+IPTV、8组参考平台未注册，宏观42组未闭环。手机只读、候选和版本保持。

- **09-10 小红书公开分享与真实媒体取得进展**：[分享合同](XIAOHONGSHU_SHARE_API_AUDIT_2026_09_10.md)。3abad7a4新增当前SSR解析，区分下播页推荐与目标房间，保留访问条件及四源/一质量；最终124/124定向、五文件严格分析通过，另有生产元数据1/1。完整2秒HLS分片1080p H.264+AAC严格解码通过，仅为外部媒体证据，尚未应用注册或双端原生验收；9组未注册、宏观42组未闭环和候选保持。手机仅只读身份/前台核对，无接管或修改。

- **09-10 战旗当前H5配置优先级与真实画质矩阵已接入**：[播放器合同](ZHANQI_PLAYER_CONTRACT_AUDIT_2026_09_10.md)。7569e40d优先h5Cdns/cdns，保留禁用列并归并同CDN/后缀身份；134/134（含真实元数据1/1）与四文件严格分析通过。当前样本仅索引2、四单元/一源身份；公开访客签名成功但FLV404、调度节点DNS/代理连接失败，无有效媒体或原生PASS。9组未注册与宏观42组未闭环保持，手机仅只读核对、候选不变。

- **09-10 战旗底层元数据合同已接入，媒体仍待验证**：[战旗审计](ZHANQI_METADATA_API_AUDIT_2026_09_10.md)。659e4715 保留目录页号/总数、三种身份、可空昵称和平台声明状态，最终140/140定向与四文件严格分析通过；真实生产元数据1/1通过。两间status=4样本的声明HLS均404，未注册可播入口；一直播仅取得DNS/TLS访问失败，未判停运。手机只读核对后保持其他应用前台；9组未注册、宏观42组未闭环，候选和版本不变。

- **09-09 Bigo/浪 Live 增量访问证据**：[补查审计](BIGO_LANGLIVE_ACCESS_AUDIT_2026_09_09.md)。Bigo 另两间样本仍要求登录；浪 Live 参考详情接口和官网经 Clash 返回 403，官方 WebView 可读但未取得当前直播合同，bundle 中 mock 媒体明确排除。两平台仍未注册，未缩减范围；转入 TTing 原生短录定位已接入链路的实际问题。

- **09-09 Bigo 目录与登录状态底层 API 已验证，仍未注册**：[元数据审计](BIGO_METADATA_API_AUDIT_2026_09_09.md)。cf704d5f 修订真实目录可空封面，保留身份与登录要求，不把 gated alive=0 误报下播；最终 78/78（含生产 Dio 探针 1/1）、三文件严格分析通过。实时目录 20 房间、所选 owner 匹配，尚无有效媒体/原生播放或录制证据。当前仍 18 直播站点 + IPTV、9 组未注册、42 大项未闭环；没有构建、安装或操作手机，候选不变。下一步补媒体合同与后续应用接线。

- **09-09 当前差距：TTing/FLEX 已注册并取得生产列表链路证据**。[应用接入](TTING_APPLICATION_INTEGRATION_AUDIT_2026_09_09.md) 201/201；[真实生产 relay](TTING_PRODUCTION_RELAY_AUDIT_2026_09_09.md) 1/1，四档共 24 份 HLS 列表通过，尚未原生解码或录制成品验收。当前源码 **18 个直播站点 + IPTV，9 组参考平台未注册**：小红书、YouTube、niconico、Bigo、战旗、一直播、企鹅电竞、浪 Live、微博直播。平台注册不替代完整能力和双端原生验收，历史 42 大项继续；下文保留各阶段取证快照。

- **09-09 OPENREC / mellow-fan 源码已注册，在线整链仍待验**：[应用审计](OPENREC_APPLICATION_INTEGRATION_AUDIT_2026_09_09.md)。实现 70a78805，接通复合身份收藏/分享、真实 HLS 画质、共享播放器/FFmpeg 请求头及配置迁移。24 文件 291/291 回归、27 文件及最终探针分析通过；DIRECT 诊断目录 200 但无可选卡片，Clash 目录 403，两条注册适配器生产探针均 0/1，未伪报为通过。当前 **17 个直播站点 + IPTV，10 组参考平台未注册**；历史矩阵仍 42 大项未闭环。未构建、安装或操作手机，无原生 PASS 增量。下方数量为各批历史快照。

- **09-09 OPENREC / mellow-fan 底层 API 已验证，应用接入待续**：[合同审计](OPENREC_API_AUDIT_2026_09_09.md)。实现 37bdd9c9；按公开频道 ID 与数字身份核对当前广播，保留三种 HLS 源族及必要请求头。真实响应暴露 limit 不保证页长，已改为明确空页才结束。最终 30 项定向 + 1 项直连生产探针通过、三文件分析无问题；Clash 目录生产探针仍为 access 失败，媒体列表 200 不等于完整代理链路通过。仍为 16 个直播站点 + IPTV、11 组未注册，无构建、设备操作或原生 PASS 增量。

- **09-08 当前状态：花椒应用已注册**：[应用审计](HUAJIAO_APPLICATION_INTEGRATION_AUDIT_2026_09_08.md)记录原生游标目录、UID 收藏/分享、配置迁移和播放/录制解析；累计 287 项定向闭环。当前源码 **16 个直播站点 + IPTV，11 组参考平台未注册**：OPENREC、小红书、TTingLive、YouTube、niconico、Bigo、战旗、一直播、企鹅电竞、浪 Live、微博直播。花椒搜索/弹幕、特殊房间与双端原生验收仍待完成。下方平台数量为各批历史快照，不作为当前总量。

- **花椒公开合同与底层 API 已验证，应用接入待续**：[花椒审计](HUAJIAO_API_AUDIT_2026_09_08.md)。真实响应复现并修订地理 point 误过滤、空标题和缺广播哨兵；真实 Dio 适配器回归补齐请求级取消收尾。34/34 定向、三文件 analyze、生产 API 1/1 通过；两页原生游标和 UID→当前广播→HLS/FLV 匹配已验。尚未平台注册、构建或原生验收，仍 15 直播站点 + IPTV、12 组未注册。下一步核验相邻 API 的同类流收尾，再接入花椒导航/收藏/播放录制。

09-08 最新状态：[克拉克拉应用接入](KILAKILA_APPLICATION_INTEGRATION_AUDIT_2026_09_08.md)已注册 UID 收藏与播放/录制；[萌星目录修订](KILAKILA_RISING_STAR_AUDIT_2026_09_08.md)补齐真实 dataType=2 行种类及官方标题。当前源码 **15 个直播站点 + IPTV，12 组参考平台未注册**；旧 8ea62ac6 包因未包含此修订暂缓安装，原生及完整能力继续。下方数量为各阶段历史快照。

09-08 克拉克拉底层增量：[API 审计](KILAKILA_API_AUDIT_2026_09_08.md)新增原生分页、匹配广播/主播、独立元数据和 FLV/HLS 解析及错误分类，33 项测试和最终三文件 analyze 通过；尚未应用注册、跨开播身份或原生验收。仍计入 13 个未注册分组，下方“尚无适配器”保留为此前取证状态。

09-08 克拉克拉增量：[公开合同审计](KILAKILA_PUBLIC_CONTRACT_AUDIT_2026_09_08.md)核对新旧详情主机、包装层不同的原生分页、历史回放错误与不透明分享跳转；取得免费公开样本的 FLV 前缀/HLS 列表。尚无适配器或原生解码，仍在13个未注册分组之内，不减少剩余计数。

09-08 应用增量：[映客应用接入](INKE_APPLICATION_INTEGRATION_AUDIT_2026_09_08.md)已注册导航、目录范围说明、迁移、分享、严格录制解析与恢复。当前源码 **14 个直播站点 + IPTV，13 组参考平台未注册**；映客原生播放/录制与完整能力继续。下方各批数量保留历史快照，不将源码接入等同全平台验收。

09-08 底层阶段：[映客公开合同](INKE_PUBLIC_CONTRACT_AUDIT_2026_09_08.md)已补官网目录、数字状态、详情缺媒体与公开精选 FLV 的实证，并新增底层 API；26 项确定性测试、三文件 analyze、生产 API 探针通过。尚未应用注册、完整媒体覆盖或原生验收，未注册分组数不因底层文件增加而减少。

后续增量：[Picarto接入审计](PICARTO_ADAPTER_AUDIT_2026_09_07.md)与[TwitCasting接入审计](TWITCASTING_ADAPTER_AUDIT_2026_09_07.md)及[猫耳应用接入审计](MISSEVAN_APPLICATION_INTEGRATION_AUDIT_2026_09_08.md)记录三组源码实施。当前源码为13直播站点加IPTV；剩余14组未注册，已注册平台的完整能力与原生验收仍分项保留。TwitCasting审计同时记录OPENREC公开请求403，OPENREC仍未实现。下文17组为实施前的固定差距快照，不表示17组已验收完成。

本轮以 d509bd8d 源码注册表为准，重新读取参考仓库远程HEAD、完整Git树（truncated=false）、README与实际注册入口，不合并或运行参考代码。

- [biliup 固定入口](https://github.com/biliup/biliup/blob/906e0f6fdb104d65989d12b76c9a6f02205384cb/crates/biliup/src/downloader/live/mod.rs)：19个站点，另有Twitch录像分支与通用适配器；辅助wbi/huya_wup模块不是独立平台。
- [bililive-go 固定入口](https://github.com/bililive-go/bililive-go/blob/ef71711a7c573b013d82fec01ee8d0609ee36aca/src/cmd/bililive/internal/init.go)：20个站点，system不计站点；README表只列17项，漏列源码已导入的快手、Twitch和小红书。源码存在/注册不保证这些参考接口现在仍可用。
- 两个HEAD与09-05相同，之前“Picarto文件504未读”的缺口本轮已补上。
- Pure Live `lib/core/sites.dart` 为10个直播站点加IPTV，共11适配器。快手已使用增量评论feed，CC/AcFun仍是EmptyDanmaku；兼容性文档旧的快手“未接入”及AcFun候选描述已更正。

本地原始证据：`local-artifacts/reference/platform-inventory-20260907/`，包含HEAD时间、完整树、README、注册入口、选定适配器源码及匿名网络结果。没有用户Cookie或令牌注入。

## 09-07 实施前差距快照

下表保留最初 17 组候选的实施前状态；当前注册数量和剩余组以页首最新增量及 [当前验收状态](ACCEPTANCE_STATUS_3_2_0.md) 为准。

下表按站点品牌合并KilaKila/红豆FM、SOOP/AfreecaTV。Pure Live的SOOP使用`sooplive.co.kr`，参考还出现`afreecatv.com`与`play.sooplive.com`；品牌合并只用于计数，旧/新域名及区域接口兼容并未因此证明。Twitch录像与通用解析单列能力，不增加直播站点数。

| 已注册的10个直播站点 | 参考对应 | 当前缺口口径 |
| --- | --- | --- |
| 哔哩哔哩、斗鱼、虎牙、抖音、快手、网易CC、YY、AcFun、Twitch | 两个参考均有 | 现有功能按各自源码/接口/原生证据分层；注册不代表全部通过 |
| SOOP / AfreecaTV | biliup AfreecaTV；bililive-go SOOP | 本项目韩国接口已接入；跨旧域名/区域合同另验 |

**还有17个未注册的平台候选分组**。它们是用户要求的平台扩展范围，不包含在历史62行/42未闭环大项中，不得悄悄以已有十个平台替代完成标准。

| 候选 | 参考 | 当前状态与具体下一步 |
| --- | --- | --- |
| Picarto | biliup | 本轮目录/详情/HLS取得200；下一步生产适配器、状态分类、分房间HLS质量及恢复夹具 |
| OPENREC | bililive-go | 前批代理样本403；保留可达性/区域条件缺口，先结构化页面与响应分类，不直接移植正则 |
| 小红书 | bililive-go | 对应Issue819；源码通过分享信息接口读取房间，本项目尚未接入，先公开分享链接与房间状态样本 |
| 映客 | biliup | 未接入；核验公开房间ID、状态、媒资及失效类型 |
| 猫耳FM | 两者 | 未接入；核验音频直播、房间/录音模式、声音文件收尾，避免强制视频轨 |
| KilaKila / 红豆FM / 克拉克拉 | 两者 | 未接入；先核验别名/域名、音频与视频房间状态 |
| TTingLive | biliup | 未接入；核验公开入口、房间身份和媒体协议 |
| YouTube | biliup | 未接入；参考依赖yt-dlp/Streamlink的部分不是现成Dart移动端能力；需处理直播/录像/账号边界 |
| TwitCasting | biliup | 未接入；核验公开频道、状态、HLS和弹幕身份 |
| niconico | biliup | 未接入；核验账号依赖、会话续期与外部工具依赖 |
| Bigo Live | biliup | 未接入；核验频道ID、可达性和实际画质列表 |
| 战旗 | bililive-go | 未接入；先确认当前站点/接口生命周期与公开在播样本，源码留存不等于现网可用 |
| 一直播 | bililive-go | 未接入；核验直播链接、房间状态和媒资 |
| 企鹅电竞 | bililive-go | 未接入；先核验站点/接口生命周期，不据参考README宣称可用 |
| 浪live | bililive-go | 未接入；核验多个域名与直播身份、区域访问条件 |
| 花椒 | bililive-go | 未接入；核验公开目录/分享、房间状态与流 |
| 微博直播 | bililive-go | 未接入；区分直播、回放和分享页，核验账号与短链接 |

其他差距：参考的通用适配器调用桌面外部工具，不能直接等同于本项目IPTV支持任意网页；历史审计提及TikTok，本次固定内置注册表没有单独TikTok适配器，应按通用解析/独立候选核验而非计为参考内置支持。定时/批量检测、音频站点、弹幕留档及后处理等功能也需逐合同对照，不机械复制服务端录播器的默认行为。

## Picarto：从源码未读推进到公开输入实证

参考 [Picarto适配器](https://github.com/biliup/biliup/blob/906e0f6fdb104d65989d12b76c9a6f02205384cb/crates/biliup/src/downloader/live/picarto.rs)。在本机确认Clash 7897监听后，仅为本次HTTP请求指定代理，没有改变应用/系统代理设置或操作手机。

| 请求 | 本轮实测 |
| --- | --- |
| `ptvintern.picarto.tv/api/explore` | HTTP200，10条，第1/6页、total51；显式adult=false，未复制参考的adult=true默认 |
| 公开频道详情 | HTTP200；选定目录中公开、在线且无标签的绘画频道，channel ID15237 |
| 多播房间映射 | getMultiStreams含4条；目标是第二条，以channelId匹配15237而非错误选第一条 |
| HLS主列表 | HTTP200 / 140B / EXTM3U；1个variant，声明1280×720、60fps、3,661,056带宽、H.264+AAC |

最后HLS结果时间05:58:43Z。该信息是播放列表声明，**不是实测解码码率、音视频播放或录制通过**；没有获取整个直播视频。当前应用仍未注册Picarto。

### 下一实施合同

1. URI解析验证精确主机和频道段，拒绝搜索/分类/相似域名；不采用未锚定的参考正则。
2. 公开目录读取有界分页、原始在线/观看含义和图片字段；分类/搜索缺公开证据时明确能力，不伪造搜索结果。
3. private、401/403、429、5xx、字段缺失与明确下播分开。参考将private及缺少loadbalancer都映射Offline，这会让监控误判，不能照搬。
4. 详情的多播流按channelId配对；验证origin主机片段，解析标准HLS属性与相对URI；稳定质量ID、多个线路与续期重新取详情。
5. 建立公开在线、明确离线、私有、限流、损坏结构、多播顺序和取消/迟到夹具，再接入导航/分享/平台能力与严格录制解析。
6. 后续串行执行代理媒体探针、Android/Windows播放、短录封装/完整解码、停止清理与长时验证。CC/AcFun弹幕缺口另外保留。

本轮只更新来源/接口证据和文档，没有应用改动、Flutter重测、构建或发布。原整体目标继续；17个扩展分组仍未实现，不因本次接口200而减少为16个。
