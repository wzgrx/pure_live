...
[Windows CPU对照诊断](WINDOWS_CPU_CONTEXT_AUDIT_2026_09_07.md)：Pure Live空页面约0.958%/0.045%/0.924%；后续相同SDK无插件静态模板Debug约0.243%/0.013%/0.288%、Release约0.306%/0.0067%/0.308%（可见/最小化/恢复）。[产品Release复验](WINDOWS_RELEASE_CPU_AUDIT_2026_09_07.md)已构建2d8e4e0c候选，结果约1.208%/0.0166%/1.330%，正常退出；WPR启动失败未取得热栈，转进程内仪表定位。不是应用修复或性能PASS。
