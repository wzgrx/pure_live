...
最新范围总览：[当前候选、剩余账目和收口顺序](ACCEPTANCE_STATUS_3_2_0.md)。干净 `67612cc9` 已完整通过 1300 测试/42 接口并构建 Android 候选，保留数据完成覆盖安装；横屏复验因前台切换停止，尚未记通过。历史账本 62 大项中 42 项 RUN/NR 不等同当前 Bug 数或完成百分比。

## 累计候选门禁（2026-09-07）

[候选审计](CUMULATIVE_CANDIDATE_AUDIT_2026_09_07.md)：干净播放器/工具箱累计源码首次全量 1520/1522，静态分析无诊断。两项旧录制测试只修订期限隔离和所有权等待，8/8 定向通过；完整重验及 Android 构建继续，尚无本阶段新包或安装。
