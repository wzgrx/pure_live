...
- Reserve **Astra Light** only for Windows client tests that genuinely need Computer Use visual interaction. Astra is especially strong at Computer Use, but its higher cost makes it a focused GUI-test resource rather than the general-purpose model. For each Windows client acceptance batch, start exactly one Astra Light task when GUI testing begins (not one task per check), then reuse that same task for every GUI check until the batch is complete. Source review, ordinary tests, builds, documentation, Android work and non-Computer-Use checks stay on the regular configured model.
