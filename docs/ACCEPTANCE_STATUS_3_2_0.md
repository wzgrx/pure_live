...
当前账本已有 20 项 PASS、36 项 RUN、6 项 NR，共 **42 个历史大项尚未闭环**。一行通常含多个动作或平台组合；此外还有单独的平台能力表、长时验证及发布门禁。部分 RUN/NR 已在后续审计产生增量证据，尚需对齐；部分 PASS 属于旧版本/旧手机。由此既不推导“当前还有 42 个 Bug”，也不推导完成百分比或最终候选已有 20 项通过。
