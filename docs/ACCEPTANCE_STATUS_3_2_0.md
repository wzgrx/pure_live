...
| Android 当前编号账本 | 46 行：16 PASS / 28 RUN / 2 NR；每行仍含多个动作和平台组合，旧包证据不自动覆盖当前源码 |
...
| 总体范围 | 当前 **21 个直播站点 + IPTV，6 组未注册**；历史大组 20 PASS / 40 RUN / 2 NR |
...
当前账本已有 20 项 PASS、40 项 RUN、2 项 NR，共 **42 个历史大项尚未闭环**。一行通常含多个动作或平台组合；此外还有单独的平台能力表、长时验证及发布门禁。部分 PASS 属于旧版本/旧手机，RUN 也可能只覆盖其中一个子场景。由此既不推导“当前还有 42 个 Bug”，也不推导完成百分比或最终候选已有 20 项通过。
