...
- 累计候选正在重新过门禁：[候选审计](CUMULATIVE_CANDIDATE_AUDIT_2026_09_07.md)。首次全量为 **1520/1522**，两项旧录制测试的短期限/清理等待已隔离，修订后独立 8/8 通过；新全量与 Android 编译尚待完成，未安装新包。


- 最新源码批次：[播放器已知房间取流/投屏](KNOWN_ROOM_LINK_ACTION_AUDIT_2026_09_07.md)，**160/160 定向回归通过，analyze 无诊断**。菜单和控制栏共用单动作门禁，离页/房间变化隔离迟到结果，复制等待确认，投屏转交选定 URL 并管理自己的子路由。物理 DLNA 接收器未实测；累计新候选构建和原生验收是下一步。
