...
The runtime controls model and reasoning effort. Reserve **Astra Light** for Windows client acceptance that genuinely requires Computer Use visual interaction. Astra is especially strong at Computer Use, but its higher cost makes it a focused GUI-test resource rather than the general-purpose model. When GUI testing begins, create exactly one Astra Light task for the Windows acceptance batch and reuse that same task for every GUI check until the batch is complete; do not create one task per screen or test case. Source review, ordinary tests, builds, documentation, Android work and other non-Computer-Use checks use the regular configured model. Agent/build files have no OpenAI API request settings, so writing model API parameters into them does not configure the running Codex session. Optimize useful context and evidence, not token count or parallelism in isolation.
