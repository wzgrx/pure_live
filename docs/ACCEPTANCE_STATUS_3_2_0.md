...
- **09-12 当前累计 Android 候选已覆盖安装并完成基础冒烟**：[完整审计](CURRENT_ANDROID_CANDIDATE_2026_09_12.md)。干净 `3e41e848` 构建/归档 arm64 Debug，288800843 B，SHA-256 `43ECD876…158D57`，签名、内容与 16 KB ELF 对齐通过；旧包与候选证书一致，安装前后 58 个状态文件逐路径/大小/SHA 完全一致，设备 `base.apk` 哈希与候选一致。当前 Bilibili 冷启动、刷新、播放、10 条可见弹幕、画质/线路、纯音频往返、PiP 恢复、致命日志和退出资源 **16/16 PASS**。#858 首页软件注入音量增加使媒体流 0→10、铃声流保持 0，随后媒体流/静音、前台、进程及 stay-awake 全部恢复；实体按钮与播放中等场景继续。宏观仍为 **20 PASS / 33 RUN / 9 NR**、42 组未闭环；未改 Root/LSP/网络/ADB 端口或发布，Astra Light 使用 0 次。
