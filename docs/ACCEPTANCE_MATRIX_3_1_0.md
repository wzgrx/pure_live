...
| A2-04 | RUN | 小窗弹幕固定预览/双栏预览实时更新，保存/恢复默认与模板状态一致 | 共享指标已统一预览与实际 Overlay；`3e41e848` K90 基线完成开→关→重启→开→重启。`63597cf1` 又统一开关整行点击与具名 toggle 语义，为七类滑块补充设置名/格式化值，并让恢复文案与 14 项默认值一致；六文件 43/43、全库 analyze 通过。覆盖安装后原生 4 个 Switch 均具名，字号/字重滑块具名；取消恢复保持关闭，确认恢复回到默认开启并跨进程保持，规范 Hive 精确恢复、应用停止。Windows GUI、颜色/后续滑块逐项交互、主弹幕模板、实际系统 PiP/小窗和长时性能继续，详见 `docs/PIP_DANMAKU_PREVIEW_FIDELITY_AUDIT_2026_09_11.md`、`docs/ANDROID_PIP_DANMAKU_SETTINGS_NATIVE_AUDIT_2026_09_12.md`、`docs/ANDROID_PIP_DANMAKU_ACCESSIBILITY_RESET_AUDIT_2026_09_12.md` |
