...
当前累计源码已经落到同签名 Android 候选并覆盖安装，安装前数据范围逐文件一致，基础播放/弹幕/音频模式/PiP 冒烟、标准流与竖屏流的普通页/沉浸/横屏/PiP 往返、首页软件音量按键路由，以及小窗弹幕总开关/预览/重启持久化补证均通过。它仍是 Debug 验收输入，不替代完整全库质量、全部设置页、22 个平台、录制、故障注入、长测和正式发布门禁；宏观状态为 **20 PASS / 34 RUN / 8 NR，共 42 组未闭环**。
