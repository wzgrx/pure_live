...
- **TLS握手停止已有源码修订与本机闭环证据**：[连接所有权审计](RECORDING_TLS_OWNERSHIP_AUDIT_2026_09_07.md)。原红测保留全部断言转入日常测试，56/56定向回归通过；直接/CONNECT握手取消、错误证书/主机名拒绝、两次3MiB+17B逐字节响应、会话隔离及清理通过。原生四个HTTP上游停止场景严格解码/输出SHA保持，暂停场景仍标记尾片舍弃。增加的TLS网络侧桥尚待Android及长期代理/功耗验收；未构建包含此改动的APK，未操作手机。
